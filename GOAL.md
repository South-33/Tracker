...
Use version 2 of `TrackingAccumulator` alongside official TrackEval. A **covered identity segment** is a contiguous annotated segment with the same public identity, no switch or cross-person ID reuse, and detections on at least 80% of its annotated frames at IoU >=0.5. Here the metric's "visible" means GT annotation present, not independently verified human visibility; review occlusion cases before making user-behavior claims. Every GT segment is in the denominator, including people never tracked. Public ID reuse invalidates segments using that ID. Also report the fraction of annotated GT person-frames belonging to successful segments, so many easy short segments cannot hide long failures.
...
When the complete trainer is restored, spend one bounded engineering check measuring steady steps/second, data wait and peak VRAM on training data. Keep mixed precision and resumable checkpoints; choose batch/unroll from the actual temporal learning question and measured memory headroom. Use accumulation if it preserves the intended optimization batch. Log effective batch, optimizer updates, distinct frames, rollout seconds and precision mode. More steps over the same few examples are not more data.

Cached frozen features/detections are appropriate for experiments that keep that perception fixed. They must not force every new idea into the same frozen representation: a temporal perception experiment needs the relevant image layers trainable and a fresh data path. The grouped annotation loader removes repeated table scans without changing labels; `lab benchmark-data` measures that preparation phase separately. Once a full loop exists, evaluate memory caching and asynchronous transfers against measured reuse/data wait. Do not add DataLoader workers, pin every tensor, or chase compilation blindly on Windows; the [PyTorch transfer guide](https://docs.pytorch.org/tutorials/intermediate/pinmem_nonblock.html) shows why transfer choices need measurement.
