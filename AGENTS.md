...
- Read `tracker.md` and run `.\lab.ps1 status`. Keep one project document, `tracker.md`, plus this lean `AGENTS.md`; the ledger owns execution state. Do not create other Markdown files. Store archived context inside evidence JSON records.
- Use `tracker.md`'s decision checklist before choosing a bet and at planned checkpoints. Prioritize the useful 80% regime; at most three exploratory experiments, including train-only diagnostics, then a substantial bet or an explicit new hypothesis. `lab` enforces the shared budget, not the scientific reasoning.
...
- Ordinary `rg` searches exclude archived source/context via `.ignore`; search provenance with `rg --no-ignore -l` first, then inspect only matching records.
