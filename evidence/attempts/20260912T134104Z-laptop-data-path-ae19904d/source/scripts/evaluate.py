...
def main():
    ...
    result, messages = evaluator.evaluate([dataset], [trackeval.metrics.HOTA(), trackeval.metrics.CLEAR({"PRINT_CONFIG": False}), trackeval.metrics.Identity({"PRINT_CONFIG": False})])
    ...
    summary = {"scope": "DanceTrack contiguous sequence; not official full-benchmark scores",
               "prediction_scope": metadata.get("scope", "unspecified; inspect prediction provenance"),
